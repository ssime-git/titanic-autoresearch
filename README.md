...
Autonomous feature engineering loop on the Titanic dataset using the **autoresearch pattern** (Karpathy).

An agent autonomously iterates on feature engineering hypotheses, testing each, analyzing results, and refining the next hypothesis—demonstrating autonomous AI research workflows.

## Quick Start

```bash
# 1. One-time setup (download data)
make setup

# 2. Run autoresearch loop
make run
...
1. **Baseline (Iteration 0)**: Trains a model without feature engineering (AUC 0.8654)
2. **Iterations 1+**: Agent proposes features, implements, trains, evaluates, and analyzes
3. **Stop condition**: Plateau (3 consecutive no-improvement iterations) OR 20 iterations max
4. **Output**: Logs all iterations to `logs/iterations.jsonl`, visualizations to `plots/`

## For Developers

### Running the Agent

Give this prompt to **any LLM agent** (Claude, other AI):
...
├── src/
│   └── autoresearch_loop.py  # Main script (agent modifies this)
├── data/
│   ├── raw/
│   │   └── titanic_original.csv  # 1309 rows, 14 columns
│   └── processed/
├── logs/
...
make clean      # Remove iteration outputs
make help       # Show help
```
...
- **Feature engineering**: See PRD suggestions in `docs/PRD.md`
